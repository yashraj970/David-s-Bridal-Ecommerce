...
const DressesProd = () => {
  ...
  const getData = () => {
    setLoading(true);
    axios
      .get(`https://639b3492d5141501974eaf28.mockapi.io/nykaa`)
      .then((data) => {
        ...
        {dresses.map((dress) => (
          <GridItem textAlign={"center"} key={dress.id}>
            ...
            <Button
              ...
              onClick={() => HandleCart(dress)}
            >
              Add to Cart
            </Button>{" "}
            <br />
            <RouterLink to={`/dressespage/${dress.id}`}>
              {" "}
              <Text _hover={{ color: "blue.500" }} color={"blue.300"}>
                See More Details
              </Text>{" "}
            </RouterLink>
          </GridItem>
...
export default memo(DressesProd);
